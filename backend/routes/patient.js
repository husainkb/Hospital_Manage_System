...
router.get('/available-slots', auth, async (req, res) => {
  try {
    ...
    // Normalize date if your DB stores Date object (optional)
    const formattedDate = new Date(date).toISOString().split('T')[0];

    // Fetch booked appointments for that doctor and date
    const bookedAppointments = await Appointment.find({ 
      doctorId, 
      date: formattedDate // ✅ make sure schema stores YYYY-MM-DD
    });
