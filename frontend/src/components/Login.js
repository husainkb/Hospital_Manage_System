    ...
    <div className="min-h-screen bg-red-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-lg shadow-lg overflow-hidden">
        <div className="bg-red-600 p-6 text-white">
          ...
          <p className="text-center text-red-100 mt-1">Access your account</p>
        </div>
        <div className="p-6">
          <div className="flex bg-red-100 rounded-lg p-1 mb-6">
            {roles.map((role) => (
              <button
                key={role.id}
                onClick={() => setSelectedRole(role.id)}
                className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-md transition-colors ${
                  selectedRole === role.id ? 'bg-red-600 text-white' : 'text-red-600'
                }`}
              >
                <role.icon size={16} />
        ...
        <div className="bg-gray-50 px-6 py-4 text-center">
          <p className="text-sm text-gray-600">
            ...
            <button onClick={() => navigate('/signup')} className="text-red-600 font-semibold hover:underline">
              Sign up
            </button>
          </p>
        </div>
